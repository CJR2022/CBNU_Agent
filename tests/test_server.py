            ...
            if not has_tool_result:
                return AIMessage(
                    ...
                    tool_calls=[
                        {
                            "name": "search_notices",
                            "args": {"query": "오늘 양성재 메뉴"},
                            "id": "call_search_notices",
                        }
                    ],
                )
            return AIMessage(
...
class _FakeChatOpenAI:
    ...
    def invoke(self, messages, config=None, **kwargs):
        ...
        if last_human and "점심" in last_human:
            return AIMessage(
                content='{"answer": "방금 양성재 메뉴 중 점심은 비빔밥입니다.", "sources": [], "confidence": "high"}'
            )
        if last_human and "메뉴" in last_human:
            return AIMessage(
                content='{"answer": "오늘 양성재 메뉴는 김치찌개, 비빔밥, 된장찌개 등이 제공됩니다.", "sources": ["https://dorm.chungbuk.ac.kr/home/sub.php?menukey=20041&type=2"], "confidence": "high"}'
            )
        return AIMessage(
            content='{"answer": "안녕하세요. 충북대학교 정보 안내 챗봇입니다.", "sources": [], "confidence": "high"}'
        )
...
def test_search_notices_returns_text(server_mod):
    ...
    fake_doc = type(
        ...
        (),
        {
            "page_content": "등록금 납부 안내",
            "metadata": {"url": "https://example.com/notice/1", "source": "main"},
        },
    )()
    mock_retriever = type(
