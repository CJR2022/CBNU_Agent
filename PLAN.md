...
>
> **원칙 3**: 식단은 **실시간 조회**가 중요하므로 별도 Tool로 두고, 공지사항들은 **RAG로 통합 검색**한다.
...
def get_dorm_menu(dorm_name: str = "개성재") -> str:
    """충북대 기숙사 오늘의 식단을 반환합니다. dorm_name: 개성재, 양성재, 양진재"""
    mapping = {"개성재": "1", "양성재": "2", "양진재": "3"}
    dorm_type = mapping.get(dorm_name, "1")
    return fetch_dorm_menu(dorm_type)

@tool
def search_notices(query: str) -> str:
    """학교/전자정보/소프트웨어/기숙사 공지사항에서 query와 관련된 내용을 검색합니다."""
...
- `server.py` 안에서 `@tool` 데코레이터로 2개 이상 도구 구현
- **긱사 메뉴 도구**: 기숙사 식단 페이지 실시간 조회
- **공지 검색 도구**: Chroma 벡터스토어에서 검색
- 각 도구 개별 테스트
...
|-------------|------|-----------|-----------|
| "오늘 양성재 메뉴 뭐야?" | 도메인: 긱사 | `get_dorm_menu(dorm_name="양성재")` | 기숙사 식단 페이지 조회 |
...
- [ ] 최소 2개의 Tool 구현 (`get_dorm_menu`, `search_notices`)
